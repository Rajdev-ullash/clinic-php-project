...
if(!empty($_POST)){
	...
	/* ----------- IMAGE UPLOAD ------------------*/
		...
		$thisfile = basename($_FILES["subimg"]["name"]);
		$p = strpos($thisfile,".");
		...
		if(isset($_POST["submit"])) {
		    $check = getimagesize($_FILES["subimg"]["tmp_name"]);
		    if($check !== false) {
		        $uploadOk = 1;
		    } else {
	...
	$query = "INSERT INTO product_image(product_id, sub_image) VALUES ('$pro_id','$imagefile')";
