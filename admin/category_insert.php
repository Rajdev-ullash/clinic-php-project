...
if(!empty($_POST)){
  ...
  // icon image
		...
		$thisfile = basename($_FILES["icn"]["name"]);
		$p = strpos($thisfile,".");
		$ext = substr($thisfile,$p);
		...
		// Check if image file is a actual image or fake image
		if(isset($_POST["submit"])) {
		    $check = getimagesize($_FILES["icn"]["tmp_name"]);
		    if($check !== false) {
		        $uploadOk = 1;
		    } else {
		        $output = "File is not an image.";
		...
		// if everything is ok, try to upload file
		    ...
		    if (move_uploaded_file($_FILES["icn"]["tmp_name"], $target_file)) {
		         $output = "Data saved";
		    } else {
		         $output = "Error";
		    }
		...
		$imagefile2 = $target_file;	

	/* -----------end ICON IMAGE UPLOAD ------------------*/


	$query = "INSERT INTO product_category(category,des,image,icon,ord) VALUES ('$title','$short_des','$imagefile','$imagefile2',$ord)";
